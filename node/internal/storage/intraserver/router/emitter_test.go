...
func TestEmitterEmit(t *testing.T) {
	...
	o.Group("when URI Finder does not return an error", func() {
		...
		o.Group("when node returns an error", func() {
			o.BeforeEach(func(t TE) TE {
				for _, nodeServer := range t.mockNodeServers {
					close(nodeServer.UpdateOutput.Ret0)
					nodeServer.UpdateOutput.Ret1 <- fmt.Errorf("some-error")
				}
				return t
			})

			o.Spec("it refetches the client", func(t TE) {
				go t.emitter.Emit(t.msgs[0])

				Expect(t, t.mockURIFinder.FromIDCalled).To(ViaPolling(HaveLen(3)))
			})
		})

		o.Group("when node returns retryable response", func() {
			o.BeforeEach(func(t TE) TE {
				for _, nodeServer := range t.mockNodeServers {
					nodeServer.UpdateOutput.Ret0 <- &intra.UpdateResponse{
						Code: intra.UpdateResponse_RetryFailure,
					}
					close(nodeServer.UpdateOutput.Ret1)
				}
				return t
		...
		o.Group("when the node returns a InvalidID", func() {
			o.BeforeEach(func(t TE) TE {
				for _, nodeServer := range t.mockNodeServers {
					nodeServer.UpdateOutput.Ret0 <- &intra.UpdateResponse{
		...
		o.Group("when the node returns a InvalidBuffer", func() {
			o.BeforeEach(func(t TE) TE {
				for _, nodeServer := range t.mockNodeServers {
					nodeServer.UpdateOutput.Ret0 <- &intra.UpdateResponse{
						Code: intra.UpdateResponse_InvalidBuffer,
					}
					close(nodeServer.UpdateOutput.Ret1)
				}
				return t
